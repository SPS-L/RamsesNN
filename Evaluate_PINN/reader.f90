module reader
    ...
    !Has been changed: now add arguments for model name: onnxModel_<model_name>.txt and onnxWeights_<model_name>.txt
    subroutine initialize_nnx(model_name)
        character(len=100), intent(in) :: model_name
        character(len=100) :: model_file_name, weights_file_name
        INTEGER :: Reason
        ...
        ! The following has been changed to include model_name in the file names
        model_file_name   = "onnxModel_"   // trim(adjustl(model_name)) // ".txt"
        weights_file_name = "onnxWeights_" // trim(adjustl(model_name)) // ".txt"

        open(10, file = model_file_name)
        open(11, file = weights_file_name)
