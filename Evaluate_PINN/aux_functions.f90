...
module aux_functions
    ...
    implicit none
    private
    public :: park_transform_xy_dq, park_transform_dq_xy, machine_solver, inf_bus_equations, assign_old_values_to_nn_input
...
! -------------------------------------------------------------------------
    subroutine machine_solver(x_in, output)
        use iso_fortran_env, only:real64
        ! --- Input variables ---
        ! (delta, omega, e_q, e_d, psi__d, psi__q, vf, Vinfty, theta_inf, Xline, Rlilne)
        real(real64), intent(in) :: x_in(11)
        ! --- Output variables ---
        ...
        real(real64) :: delta, omega, e_q, e_d, psi__d, psi__q, vf, Vinfty, theta_inf, Xline, Rline
        ...
        ! Sanity check
        if (size(x_in) /= 11) error stop "machine_solver: x_in must have length 11"
...
! Subroutine to assign values to the input of the neural network
    subroutine assign_old_values_to_nn_input(pinn_input, previous_timstep_vbls)
        use iso_fortran_env, only:real64
        implicit none
        real(real64), intent(inout) :: pinn_input(1,12)
        real(real64), intent(in)    :: previous_timstep_vbls(1,12)
        
        ! Assign the values to be kept from previous timestep
        ! delta(2), omega(3), e_q(4), e_d(5), psi__d(6), psi__q(7), vf(8), Vinfty(9), Vinftyang(10)
